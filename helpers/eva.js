import { map, template as t, union } from 'lodash'
...
prepare = async (props) => {
  ...
  let repo = {owner: 'akveo', repo: 'eva-icons', path: 'package/icons/fill', ref: 'master'}
  ...
  bundle            = files.map(x => 'eva-' + x.split('.').slice(0, -1).join('.'))
  let style_fill    = bundle.map(x => x + '-fill')
  let style_outline = bundle.map(x => x + '-outline')

  bundle = union(style_fill, style_outline)
